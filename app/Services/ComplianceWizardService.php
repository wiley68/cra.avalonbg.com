...
class ComplianceWizardService
{
     ...
     *         percent: int,
     *         optional_total: int,
     *         optional_complete: int,
     *         optional_dismissed: int
     *     },
    ...
    public function build(Product $product): array
    {
        ...
        // After the required spine is done, do not pin "current" on an early optional
        // step — that hid later completed steps (18–24) from the completed list.
        $currentKey = $requiredComplete
            ? null
            : $this->resolveCurrentStepKey($statuses, $dismissed);
    ...
    /**
     * @param  list<array{required: bool, is_complete: bool, is_dismissed: bool}>  $steps
     * @return array{
     *     required_total: int,
     *     required_complete: int,
     *     percent: int,
     ...
     * }
     */
    public function buildProgress(array $steps): array
    {
        ...
        $requiredComplete = count(array_filter(
            $required,
            static fn(array $step): bool => $step['is_complete'],
        ));
        $optionalComplete = count(array_filter(
            $optional,
            static fn(array $step): bool => $step['is_complete'],
        ));
        $optionalDismissed = count(array_filter(
            $optional,
            static fn(array $step): bool => $step['is_dismissed'] && !$step['is_complete'],
        ));
        ...
        return [
            ...
            'percent' => $requiredTotal === 0
                ? 0
                : (int) round(($requiredComplete / $requiredTotal) * 100),
            'optional_total' => count($optional),
            'optional_complete' => $optionalComplete,
            'optional_dismissed' => $optionalDismissed,
        ];
    }
    ...
    private function statusForKey(Product $product, string $key, array $moduleDetails): string
    {
        return match ($key) {
            ...
            'sdl' => $this->sdlWizardStatus($moduleDetails),
            default => $this->normalizeModuleStatus($moduleDetails[$key]['status'] ?? 'empty'),
        };
    }

    /**
     * Align with readiness: SDL N/A (out of scope / no release in flight) must not block the spine.
     *
     * @param  array<string, array{status: string, section: string, summary: string}>  $moduleDetails
     * @return 'empty'|'complete'|'attention'|'critical'|'na'
     */
    private function sdlWizardStatus(array $moduleDetails): string
    {
        $summary = (string) ($moduleDetails['sdl']['summary'] ?? '');

        if (in_array($summary, ['not_required', 'no_release_in_progress'], true)) {
            return 'na';
        }

        return $this->normalizeModuleStatus($moduleDetails['sdl']['status'] ?? 'empty');
    }
