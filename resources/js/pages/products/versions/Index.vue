...
import {
    ...
    createProductVersionColumns,
} from './columns';
import type { ProductVersionListItem } from './columns';
...
const { rows, pagination, loading, search, fetch } =
    useApiTable<ProductVersionListItem>({
        endpoint: versionsApiIndex(props.product.id).url,
        initial: {
            page: 1,
            rowsPerPage: 10,
...
const columnTitleMap = computed(() => createProductVersionColumnTitleMap(t));

const requestDeleteVersion = (versionId: number): void => {
    versionToDelete.value = versionId;
...
const confirmDelete = (): void => {
    if (versionToDelete.value === null) {
        return;
    }

    const versionId = versionToDelete.value;
    versionToDelete.value = null;
    showDeleteDialog.value = false;
...
const handleSortingChange = (sorting: SortingState) => {
    ...
    pagination.value.sortBy = primary?.id ?? 'version_number';
...
<template>
    <Head :title="t('products.versions.index_title')" />
    ...
    <div class="space-y-6">
        ...
        <DataTable
            ...
            :search-placeholder="t('products.versions.search_placeholder')"
            server-side
