...
const form = useForm({
    title: props.risk.title,
    asset: props.risk.asset ?? '',
    threat: props.risk.threat ?? '',
    weakness: props.risk.weakness ?? '',
    attack_scenario: props.risk.attack_scenario ?? '',
    category: props.risk.category,
    likelihood: props.risk.likelihood,
    impact: props.risk.impact,
    residual_likelihood: (props.risk.residual_likelihood ?? '') as number | '',
    residual_impact: (props.risk.residual_impact ?? '') as number | '',
    treatment: props.risk.treatment,
    treatment_plan: props.risk.treatment_plan ?? '',
    status: props.risk.status,
    owner_user_id: (props.risk.owner_user_id ?? '') as number | '',
    deadline: props.risk.deadline ?? '',
    product_version_id: (props.risk.product_version_id ?? '') as number | '',
    control_ids: [...props.risk.control_ids],
    requirement_ids: [...props.risk.requirement_ids],
});
...
<template>
    <Head :title="t('products.risks.edit_title')" />
            ...
            <fieldset :disabled="!canManage" class="space-y-5">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div class="grid gap-2">
                        ...
                        <InputError
                            :message="form.errors.residual_likelihood"
                        />
                    </div>
                    <div class="grid gap-2">
                        <FieldLabel
