...
import type { ProductVulnerabilityListItem } from './columns';
import { index as productVulnerabilitiesApiIndex } from '@/routes/internal/products/vulnerabilities';
import {
...
const { rows, pagination, loading, search, fetch } =
    useApiTable<ProductVulnerabilityListItem>({
        endpoint: productVulnerabilitiesApiIndex(props.product.id).url,
        initial: {
            page: 1,
            rowsPerPage: 10,
            sortBy: 'title',
...
const columnTitleMap = computed(() =>
    createProductVulnerabilityColumnTitleMap(t),
);

const requestDelete = (vulnerabilityId: number): void => {
    vulnerabilityToDelete.value = vulnerabilityId;
...
const cancelDelete = (): void => {
    vulnerabilityToDelete.value = null;
    showDeleteDialog.value = false;
};

const confirmDelete = (): void => {
    if (vulnerabilityToDelete.value === null) {
        return;
    }

    const id = vulnerabilityToDelete.value;
    vulnerabilityToDelete.value = null;
    showDeleteDialog.value = false;
...
<template>
    <Head :title="t('products.vulnerabilities.index_title')" />
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-xl font-semibold">
                    {{ t('products.vulnerabilities.title') }}
                </h1>
                <p class="text-sm text-muted-foreground">
                    {{ t('products.vulnerabilities.subtitle') }}
                </p>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <Button v-if="canManage" as-child>
                    <Link
                        :href="createProductVulnerability(props.product.id)"
        ...
        <DataTable
            ...
            :search-placeholder="
                t('products.vulnerabilities.search_placeholder')
            "
            server-side
