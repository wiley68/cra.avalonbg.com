...
import { createSdlRunColumnTitleMap, createSdlRunColumns } from './columns';
import type { SdlRunListItem } from './columns';
import { index as productSdlApiIndex } from '@/routes/internal/products/sdl';
import {
    create as createSdlRun,
...
const runToDelete = ref<number | null>(null);
const versionFilter = ref('__all__');

const { rows, pagination, loading, search, fetch } =
    useApiTable<SdlRunListItem>({
        endpoint: productSdlApiIndex(props.product.id).url,
        initial: {
            page: 1,
            rowsPerPage: 10,
            sortBy: 'title',
...
const columnTitleMap = computed(() => createSdlRunColumnTitleMap(t));

const requestDelete = (runId: number): void => {
    runToDelete.value = runId;
...
<template>
    <Head :title="t('products.sdl.index_title')" />
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-xl font-semibold">
                    {{ t('products.sdl.title') }}
                </h1>
                <p class="text-sm text-muted-foreground">
                    {{ t('products.sdl.subtitle') }}
                </p>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <Button v-if="canManage" as-child>
                    <Link
                        :href="createSdlRun(props.product.id)"
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            >
                <SelectTrigger class="w-55">
                    <SelectValue
                        :placeholder="t('products.sdl.filter_version')"
                    />
                </SelectTrigger>
                <SelectContent>
                    <SelectItem value="__all__">
                        {{ t('products.sdl.filter_all_versions') }}
                    </SelectItem>
                    <SelectItem value="__none__">
                        {{ t('products.sdl.filter_product_wide') }}
                    </SelectItem>
                    <SelectItem
        ...
        <DataTable
            ...
            :search-placeholder="t('products.sdl.search_placeholder')"
            server-side
