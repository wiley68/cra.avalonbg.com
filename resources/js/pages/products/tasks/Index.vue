...
import {
    createProductTaskColumnTitleMap,
    createProductTaskColumns,
} from './columns';
import type { ProductTaskListItem } from './columns';
import { index as productTasksApiIndex } from '@/routes/internal/products/tasks';
import {
    create as createProductTask,
...
const { rows, pagination, loading, search, fetch } =
    useApiTable<ProductTaskListItem>({
        endpoint: productTasksApiIndex(props.product.id).url,
        initial: {
            page: 1,
            rowsPerPage: 10,
            sortBy: 'title',
...
const columnTitleMap = computed(() => createProductTaskColumnTitleMap(t));
...
<template>
    <Head :title="t('products.tasks.index_title')" />
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                ...
                <h1 class="text-xl font-semibold">
                    {{ t('products.tasks.title') }}
                </h1>
                <p class="text-sm text-muted-foreground">
                    {{ t('products.tasks.subtitle') }}
                </p>
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <Button v-if="canManage" as-child>
                    <Link
                        :href="createProductTask(props.product.id)"
        ...
        <DataTable
            ...
            :search-placeholder="t('products.tasks.search_placeholder')"
            server-side
