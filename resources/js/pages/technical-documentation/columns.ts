...
export type OrgTechnicalDocumentationListItem = {
    id: number;
    title: string;
    status: string;
...
export function createOrgTechnicalDocumentationColumnTitleMap(
    t: TranslateFn,
): Record<string, string> {
    return {
        id: t('products.technical_documentation.columns.id'),
        title: t('products.technical_documentation.columns.title'),
        product_name: t('technical_documentation.columns.product'),
...
export const createOrgTechnicalDocumentationColumns = ({
    t,
    canManage,
    onDelete,
}: {
    t: TranslateFn;
    canManage: boolean;
    onDelete: (packageId: number, productId: number) => void;
}): ColumnDef<OrgTechnicalDocumentationListItem>[] => {
    return [
        {
            accessorKey: 'id',
            header: ({ column }) =>
        ...
        },
        {
            ...
            header: ({ column }) =>
                sortableHeader(
                    t('technical_documentation.columns.product'),
                    column,
                ),
            cell: ({ row }) =>
                h(
