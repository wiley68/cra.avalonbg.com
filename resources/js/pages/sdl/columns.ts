...
export function createOrgSdlRunColumnTitleMap(
    t: TranslateFn,
): Record<string, string> {
    return {
        id: t('products.sdl.columns.id'),
        title: t('products.sdl.columns.title'),
        product_name: t('sdl.columns.product'),
...
export const createOrgSdlRunColumns = ({
    t,
    canManage,
    onDelete,
}: {
    t: TranslateFn;
    canManage: boolean;
    onDelete: (runId: number, productId: number) => void;
}): ColumnDef<OrgSdlRunListItem>[] => {
    return [
        {
            accessorKey: 'id',
            header: ({ column }) =>
