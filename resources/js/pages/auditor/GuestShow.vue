...
const expiresLabel = computed(() => {
    if (!props.guest.expires_at) {
        return null;
    }

    return new Date(props.guest.expires_at).toLocaleString();
});
</script>
...
<template>
    ...
    <div class="min-h-screen bg-background">
        ...
        <main class="mx-auto w-full max-w-5xl space-y-6 px-6 py-8">
            <p
                class="rounded-md border bg-muted/40 px-4 py-3 text-sm text-muted-foreground"
            >
                {{ t('auditor.guest.view_only_banner') }}
            </p>

            <div class="space-y-2">
                <p class="text-sm font-medium text-muted-foreground">
                    {{ t('auditor.guest.title') }}
                </p>
                ...
                <p class="max-w-3xl text-sm text-muted-foreground">
                    {{ t('auditor.guest.disclaimer') }}
                </p>
                <p v-if="expiresLabel" class="text-sm text-muted-foreground">
                    {{ t('auditor.guest.expires') }}: {{ expiresLabel }}
                </p>
            </div>
                ...
                <div class="grid gap-3 md:grid-cols-2">
                    ...
                    >
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <Badge
                                :variant="
                                    readinessStatusVariant(section.status)
                                "
                            >
                                {{
                                    t(
                                        `products.readiness.status.${section.status}`,
                                    )
                                }}
                            </Badge>
                        </div>
